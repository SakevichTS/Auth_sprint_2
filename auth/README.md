...
https://github.com/akkomiss/Auth_sprint_1

# 1 миграции. 
в папке auth-service выполгнить поочередно команды:  
...
API станет доступен по адресу http://127.0.0.1:8081/api/openapi#/

# 3 создание роли администратора. 
docker compose exec auth1 python -m src.cli --login admin --email admin@example.com --password secret
