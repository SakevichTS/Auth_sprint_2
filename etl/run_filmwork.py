...
def process_source(config: dict, producers: dict[str, PostgresProducer], enricher: PostgresEnricher, merger: PostgresMerger, transformer: PostgresTransformer, redis_connection: Redis):
    ...
    if film_work_ids:
        ...
        transformed_data = transformer.transform_movies(raw_data)
            ...
            while True:
                try:
                    with pg_conn_context(**settings.pg.to_dict()) as p_conn:
                        film_cfg = next(cfg for cfg in settings.producer_configs if cfg.source_type == 'film_work')
                        film_state = State(RedisStorage(redis_connection, film_cfg.state_key))
                        producers = {
                            'film_work': PostgresProducer(p_conn, film_state, film_cfg.table, settings.batch_size)
                        }
                        ...
                        for config in settings.producer_configs:
                            process_source(film_cfg.model_dump(), producers, enricher, merger, transformer, redis_connection)

                except OperationalError as e:
                    logging.warning(f"Не удалось подключиться к PostgreSQL в этом цикле. Повтор через 5 секунд. Ошибка: {e}")

                load_data_to_es(loader, redis_connection, 'processed_movies_queue')
